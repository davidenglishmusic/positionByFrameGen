...
describe PositionByFrameGen do
  ...
  before :all do
    ...
    @testObject002 = PositionByFrameGen.new([2,7],[-8,-1],0,10)
  end
  ...
  describe "calculateMidpoint" do
    ...
    it "returns [-3,3] when the starting coordinates are [2,7] and the end coordinates are [-8,-1]" do
      expect(@testObject002.calculateMidpoint()).to eq([-3,3])
    end
  end
  ...
  describe "getAllFrameCoordinates" do
    ...
    it "returns an array of each frame and it's coordinates when the starting coordinates are [2,7] and the end coordinates are [-8,-1]" do
      expect(@testObject002.getAllFrameCoordinates()).to eq([[0, [2, 7]], [1, [1.0, 6.804226065180615]], [2, [0.0, 6.23606797749979]], [3, [-1.0, 5.3511410091698925]], [4, [-2.0, 4.23606797749979]], [5, [-3.0, 3.0]], [6, [-4.0, 1.7639320225002104]], [7, [-5.0, 0.6488589908301075]], [8, [-6.0, -0.2360679774997898]], [9, [-7.0, -0.8042260651806141]], [10, [-8, -1]]])
    end
  end
